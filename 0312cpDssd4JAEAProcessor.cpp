...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(xTrace.HasValue("filterEnergyCal"))
	  EnergyF = xTrace.GetValue("filterEnergyCal");
	if(yTrace.HasValue("filterEnergyCal"))
	  EnergyB = yTrace.GetValue("filterEnergyCal");// calibrated energy delivered;
	*/
	...
	{
	  {
	    ...
	    // scale for er=10;
	    //double scaleF = 4.917; 
	    //double scaleB = 4.9339; 

	    
	    // scale for er=20;
	    double scaleF = 4.1306;
	    double scaleB = 4.1216;
	    ...
	    /*
	    cout << endl;
	    cout << "============== SINGLE PULSE =============" << endl;
	    //cout << "UNCALIBRATED" << endl;
	    cout << "Front energy: " <<  filterEnergyF << "; Front position: " << singlePosF << "; Front time: " << singleTimeF << endl;
	    cout << "Back energy: " <<  filterEnergyB << "; Back position: " << singlePosB << ";  Back time: " << singleTimeB << endl;
	    cout << endl;
	    */
	    // if((filterEnergyF*scaleF -3000)*(filterEnergyF -10000) < 0) // range for energy: 3000~10000 keV; 
	      {
	    ofstream outfile;
	    outfile.open("results.txt", std::ofstream::out | std::ofstream::app);// output information of a single pulse to a text file; 
	    
	    //outfile << "============== SINGLE SIGNAL =============" << endl;
	    //cout << "UNCALIBRATED" << endl;
	    //outfile << "Front energy: " <<  filterEnergyF << "; Front position: " << preAlphaPosF << endl;//<< "; Front time: " << preAlphaTimeF << endl;
	    //outfile << "Back energy: " <<  filterEnergyB << "; Back position: " << preAlphaPosB << endl;//"; Back time: " << preAlphaTimeB << endl;
