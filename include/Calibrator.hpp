...
class Calibrator {
    ...
    private:
        ...
        /** Linear plus Linear calibration,
         * parameters are assumed to be sorted
         * from the lowest order to the highest
         * f(x) = par0 + par1*x and in a higher range,
         * g(x) = par2 + par3*x
         */
 	double ModelLinLin(const std::vector<double>& par,
                                double raw) const;

        /** Linear plus Log calibration,
         * parameters are assumed to be sorted
         * from the lowest order to the highest
         * f(x) = par0  + par1 * x and in a higher range,
         * g(x) = exp(par2+par3*x)
         */
 	double ModelLinLog(const std::vector<double>& par,
                                double raw) const;
};
