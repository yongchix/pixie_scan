...
class JAEAEvent {
    public:
  JAEAEvent();
  JAEAEvent(double energy, double energy2,double time, int mwpc, double mwpcTime, 
  ...
  double get_time() const {
	...
	double get_mwpcTime() const {
        ...
        void set_energy2(double energy2) {
            energy2_  = energy2;
        }
...
public:
  ...
  ~JAEACorrelator();
  bool add_event(JAEAEvent& event, int x, int y, Plots& histo, bool hasVETO,bool hasNaI,bool hasPin);
  void human_event_info(JAEAEvent& event, std::stringstream& ss, double clockStart);
private:
  int size_x_;
  int size_y_;
  std::deque<JAEAEvent>** pixels_;
  bool flush_chain(int x, int y, Plots& histo,bool hasVETO,bool hasNaI,bool hasPin);
};
