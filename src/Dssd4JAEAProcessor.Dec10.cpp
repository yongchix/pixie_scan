...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(hasFront && hasBack){
	  ...
	  plot(DD_DSSDIFRONT_POSENERGY,xEnergy/4,xPosition); 
	  plot(DD_DSSDIBACK_POSENERGY,yEnergy/4,yPosition); 
	...
	JAEAEvent jaeaevent = JAEAEvent(yEnergy, time, mwpc, mwpcTime,
					hasBeam, hasVeto, hasEscape, unknown_jaea);
