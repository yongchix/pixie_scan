...
bool SheCorrelator::flush_chain(int x, int y, Plots& histo){
    ...
    if (alphas >= 2 && abs(VRecoilE*1e-3-11)<=5 && alphaE[1] > 9000 && (alphaTime[1]-VRecoilTime) < 1.0 && (alphaTime[1]-VRecoilTime) > 0 ){//In units of s/10
    cout << "INTERESTING alpha!!" << endl;
//    cout << VRecoilTime << ' ' << alphaTime[1]<< ' ' << (alphaTime[1]-VRecoilTime) << endl;
	...
	while ((alphaE[i]>0) & ((alphaTime[i]-VRecoilTime)<=10000)){
	    ittr =0;
//	    cout <<alphaE[i]  << endl;
//        cout << VRecoilTime << ' ' << alphaTime[i] << endl;

     	    histo.Plot(dammIds::dssd4she::DD_CHAIN_ALPHA_V_ALPHA,alphaE[1]/10,alphaE[i+1]/10);
	    while (ittr <= (alphaTime[i]-VRecoilTime)*1e6 && ittr <16000) {
    ...
    if ((fissionE > 0) && (fissionTime-VRecoilTime > 0) ){
        ittr=0;
        while (ittr <= (fissionTime-VRecoilTime) && ittr < 5000) {
          histo.Plot(dammIds::dssd4she::DD_CHAIN_NUM_ALPHA,fissionE/200,ctra);
          histo.Plot(dammIds::dssd4she::DD_CHAIN_NUM_ALPHA,fissionE/200+1,ctra);
          ittr++;    
        }
	}
	ctra++;      
    }
    if (fissionE > 0 && (fissionTime - VRecoilTime)*1e3 <16)  { 
...
//    cout << (fissionTime - VRecoilTime) << endl;
	...
	ittr =0;
	//cout << (fissionTime -VRecoilTime)*1e6 << endl;
