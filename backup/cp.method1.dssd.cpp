...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  //--------------- Nai Events ------------------------------------
  static vector<NaiEvent> vecNaiEvents;
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if( //xEnergy < 2730 && yEnergy < 2730
	    ...
	    //&& !hasNaI                                                 // remove the anti-gate on gamma                                                               
	    )          
	  {
	    if( chain[1][x][y].time == 0)
	      // if #1 signal is blank && !hasNaI
	      // signal is assumed to be alpha from 113Ba
	      // signal #1 should not have gamma ray
	      {
		if(!hasNaI) // can it have gamma radiation? 
		  {
	      ...
	      if( (time - chain[1][x][y].time) >= 0                    // positive time interval
		  && (time - chain[1][x][y].time)*pow(10, -8) < 0.0300 // time window = 30ms
		  )  
		{
		  ...
		  if( (chain[1][x][y].energyF < 1010 && chain[1][x][y].energyF > 880)
		      && (xEnergy - 2099)*(xEnergy - 1947) < 0 
		      ) 
		  
		  /*
		  if( (chain[1][x][y].energyF < 700 && chain[1][x][y].energyF > 300)
		      && (xEnergy - 250 )*(xEnergy - 700 ) < 0 
		      ) // for test only; 
		  */
		    // zone0 xc 880 1010 yc 2099 2251
		    // zone0-1 xc 880 1010 yc 1947 2099
		    // zone0-2 xc 880 1010 yc 1795 1947
		    {
		      sumSignal = 1; // tells nai processor;
		      sumSignalTime = time;
		      sumSignalEnergy = xEnergy;

		      plot(D_LOGDECAYTIME_0, 10*log((time - chain[1][x][y].time)*pow(10, -5))/log(10.) ); // 709; linear scale

		      //cout << endl << "====================================================="
		      //   << "TIME INTERVAL: "<< (time - chain[1][x][y].time)*pow(10, -5) << endl
		      //   << endl << "=====================================================" << endl << endl;
		      
		      ofstream outfile;
		      outfile.open("method1-DSSD-NaI.txt", ios::app);
		      outfile << endl << "======================================" << endl
			      << "1st DSSD: " << setprecision(6) << energy1F << ", " << setprecision(15) <<  chain[1][x][y].time << endl
			      << "2nd DSSD: " << setprecision(6) << xEnergy << ", " << setprecision(15) << time << endl
			      << " >> time interval(DSSD): " << (time - chain[1][x][y].time)*pow(10, -5) << endl 
			      << "NaI signals" << " >> " << endl;
		      


		      /* now handle NaI signals
		       */

		      int point = -1; 
		      double pluginEnergy = 0; 

		      int lengthVec = vecNaiEvents.size();
		      if(lengthVec > 0)
			{
			  for(int i = 0; i < lengthVec; i++)
			    if(vecNaiEvents.at(i).time - time > 0)
			      { point = i-1; break; } // record the point where nai signals is no longer prior to dssd signals;
			  if(point >= 0)
			    vecNaiEvents.erase(vecNaiEvents.begin(), vecNaiEvents.begin()+point); // erase nai signals;
			}

		      /*
		      lengthVec = vecNaiEvents.size();
		      if(lengthVec > 0)
			{
			  for(int i = 1; i < lengthVec; i++)// handle the rest signals after first purge
			    if(vecNaiEvents.at(i).time - vecNaiEvents.at(0).time > 3) // if time interval longer than 20ns;
			      { point = i; break; }
			  vecNaiEvents.erase(vecNaiEvents.begin()+point, vecNaiEvents.end()); // erase nai signals;
			}
			
		      lengthVec = vecNaiEvents.size();
		      if(lengthVec > 0)
			{
			  int interval = vecNaiEvents.at(0).time - time;
			  plot(D_INTVL_DSSD_NAI, interval + 100); // 715
			  plot(DD_INTVL_DSSD, interval + 100, xEnergy); // 716

			  if(vecNaiEvents.at(0).channel < 4) // if plug-in signal
			    {
			      for(int i = 0; i < lengthVec; i++)
				if(vecNaiEvents.at(i).channel < 4)
				  pluginEnergy = pluginEnergy + vecNaiEvents.at(i).energy; // sum over plugin signals
			      plot(DD_INTVL_NAI, interval + 100, pluginEnergy); // 717
			    }
			  else // if barrel signal
			    plot(DD_INTVL_NAI, interval + 100, vecNaiEvents.at(0).energy); // 717
			}  
		      ...
		      */
		      ...
		      for(int i = 0; i < vecNaiEvents.size(); i++)
                        //vecNaiEvents.at(i).Output();
			outfile << setprecision(6) << vecNaiEvents.at(i).energy << ", " 
				<< setprecision(15) << vecNaiEvents.at(i).time << ", "
				<< setprecision(2) << vecNaiEvents.at(i).channel << endl;
		      ...
		      */
		      ...
		      vecNaiEvents.erase(vecNaiEvents.begin(), vecNaiEvents.end()); // by Yongchi Xiao; 06/16/2015
	...
	if(canFillDT)
	  {
	    ...
	    /*
	    // ---------------- by Yongchi Xiao; 06/16/2015 -----------------------
	    for(int i = 0; i < 8; i++)
	      {
		cout << setw(10) << i
		     << setw(10) << groupNaiEvents[i].energy
		  //<< setw(10) << groupNaiEvents[i].time
		     << endl;
	      }
	    cout << endl;
	    //---------------------------------------------------------------------
