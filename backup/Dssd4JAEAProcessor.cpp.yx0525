...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  const int intervalBins = SA; // 1024
  ...
  DeclareHistogram2D(DD_CORRELATION_MATRIX_100F, decayEnergyBins, decayEnergyBins, "correlated decays in 100ms - F");// 707
  DeclareHistogram2D(DD_CORRELATION_MATRIX_100B, decayEnergyBins, decayEnergyBins, "correlated decays in 100ms - B");// 708
	...
	if( //xEnergy < 2730 && yEnergy < 2730
	    ...
	    )          
	  {
		...
		chain[1][x][y].time = time;
	      ...
	      if( (time - chain[1][x][y].time)*pow(10, -8) < 1.000 
		  && (time - chain[1][x][y].time)*pow(10, -8) >= 0)  // a signal coming in 1000ms after the 1st decay;
		  // time interval: 0.1, 0.3, 0.5, 1.0s
		{
		  plot(DD_CORRELATION_MATRIX_1000F, chain[1][x][y].energyF, xEnergy); // 713
		  plot(DD_CORRELATION_MATRIX_1000B, chain[1][x][y].energyB, yEnergy); // 714
		  plot(DD_CORRELATION_INT_DECAY_1000F, chain[1][x][y].energyF, (time - chain[1][x][y].time)*pow(10, -5) ); // 721
		  plot(DD_CORRELATION_INT_DECAY_1000B, chain[1][x][y].energyB, (time - chain[1][x][y].time)*pow(10, -5) ); // 722
		  /*
		  // by Yongchi Xiao; 05/22/2015
		  if( (chain[1][x][y].energyF < 3000 && xEnergy < 3000)
		      && (chain[1][x][y].energyB < 3000 && yEnergy < 3000)
		      )
		     cout << "event of interest :: mult_mwpc = " << mult_mwpc << endl;
		  */

		  // even if mult_mwpc == 0, some unwanted events will appear;
		  // shall we depend on MWPC energy? 
		  
		  if( (time - chain[1][x][y].time)*pow(10, -8) < 0.500) // time window = 500ms
		    {
		      plot(DD_CORRELATION_MATRIX_500F, chain[1][x][y].energyF, xEnergy); // 711
		      plot(DD_CORRELATION_MATRIX_500B, chain[1][x][y].energyB, yEnergy); // 712
		      //plot(DD_CORRELATION_INT_DECAY_500F, chain[1][x][y].energyF, (time - chain[1][x][y].time)*pow(10, -5) ); // 719
		      //plot(DD_CORRELATION_INT_DECAY_500B, chain[1][x][y].energyB, (time - chain[1][x][y].time)*pow(10, -5) ); // 720
		      ...
		      if( (time - chain[1][x][y].time)*pow(10, -8) < 0.300 ) // time window = 300ms
			{
			  ...
			  //plot(DD_CORRELATION_INT_DECAY_300F, chain[1][x][y].energyF, (time - chain[1][x][y].time)*pow(10, -5) ); // 717
			  //plot(DD_CORRELATION_INT_DECAY_300B, chain[1][x][y].energyB, (time - chain[1][x][y].time)*pow(10, -5) ); // 718
			  
			  if( (time - chain[1][x][y].time)*pow(10, -8) < 0.100 )// time window = 100ms
			    {
			      plot(DD_CORRELATION_MATRIX_100F, chain[1][x][y].energyF, xEnergy); // 707
			      // cout << "707: " << cnt707++ << endl;
			      plot(DD_CORRELATION_MATRIX_100B, chain[1][x][y].energyB, yEnergy); // 708
			      //cout << "708: " << cnt708++ << endl;

			      //plot(DD_CORRELATION_INT_DECAY_100F, chain[1][x][y].energyF, (time - chain[1][x][y].time)*pow(10, -5) ); // 715
			      //cout << "715: " << cnt715++ << endl;
			      //plot(DD_CORRELATION_INT_DECAY_100B, chain[1][x][y].energyB, (time - chain[1][x][y].time)*pow(10, -5) ); // 716
			      //cout << "714: " << cnt716++ << endl;
			    }
			}
		    }
