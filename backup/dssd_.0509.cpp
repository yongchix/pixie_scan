...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  // delay correlations
  DeclareHistogram2D(9, 3000, 50, "decays-F gated on 2 PINs"); // 709                                                                                                
  DeclareHistogram2D(10, 3000, 50, "decays-B gated on 2 PINs"); // 710
  // prompt correlations
  DeclareHistogram2D(11, 3000, 50, "decays-F gated on 2 PINs+511"); // 711
  DeclareHistogram2D(12, 3000, 50, "decays-B gated on 2 PINs+511");; // 712
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector<ChanEvent*>::const_iterator it = naiEvents.begin(); it != naiEvents.end(); it++) 
    {
      ...
      SimpleEvent se;
      se.AssignValue(naiTime, calEnergy, number, subtype); 
      vecNaiEventsAll.push_back(se);
    } // by Yongchi Xiao
  ...
  for(vector<SimpleEvent>::iterator ipf = vecPinF.begin();
	...
	ipb != vecPinB.end();
	ipb++){
      if( abs(ipf->time - ipb->time) < 6 ){ // taken as coincidence                                                                                                                                    
	doublePin = true;
	break;
      }
    }if(doublePin) break;
  ...
  // filter nai events                                                                                                                                                                                 
  for(int i = 0; i < vecNaiEventsAll.size(); i++){
    SimpleEvent se = vecNaiEventsAll.at(i);
    double energy_ = se.energy;
    int channel_ = se.channel;
    switch(channel_){
    case 0: if(energy_ < 295 && energy_ > 149){ found511 = true;} break;
    case 1: if(energy_ < 295 && energy_ > 149){ found511 = true;} break;
    case 2: if(energy_ < 295 && energy_ > 149){ found511 = true;} break;
    case 3: if(energy_ < 295 && energy_ > 149){ found511 = true;} break;
    case 4: if(energy_ < 663 && energy_ > 113){ found511 = true;} break;
    case 5: if(energy_ < 663 && energy_ > 113){ found511 = true;} break;
    case 6: if(energy_ < 663 && energy_ > 113){ found511 = true;} break;
    case 7: if(energy_ < 663 && energy_ > 113){ found511 = true;} break;
    }
    if(found511) break;
  }
  // filter real decays based on results above
  for(vector< pair<SimpleEvent, SimpleEvent> >::iterator id = vecDecays.begin();
      id != vecDecays.end();
      id++){
    plot(13, id->first.energy, id->first.channel); // 713
    plot(14, id->second.energy, id->second.channel); // 714
    // conditional plotting
    if(doublePin){
      plot(9, id->first.energy, id->first.channel); // 709
      plot(10, id->second.energy, id->second.channel); // 710
      
      if(found511){
	plot(11, id->first.energy, id->first.channel); // 711
	plot(12, id->second.energy, id->second.channel); // 712
      }
    }
