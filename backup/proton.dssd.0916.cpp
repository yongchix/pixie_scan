...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(DD_DELTAT_DSSDENERGY, 1024*8, 1024*8, "Time interval vs. DSSD - E"); // 710
  DeclareHistogram2D(DD_DELTAT_NAIENERGY_IN, 1024, 1024*8, "Time interval vs. NaI - E - in"); // 711
  DeclareHistogram2D(DD_DELTAT_NAIENERGY_OUT, 1024, 1024*8, "Time interval vs. NaI - E - out"); // 712
  //-----------------------------------------------------------------------------------------
		...
		if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		   ...
		   )
		  {
		    for(int i = 0; i < vecNaiEvents.size(); i++)
		      {
			if((time - vecNaiEvents.at(i).time)*pow(10, -2) < 30 
			   && (time - vecNaiEvents.at(i).time) > 0) // time window = 30us, half-life of 113Cs is 13us;
			  {
			    int channel_ = vecNaiEvents.at(i).channel;
			    double eCheck = vecNaiEvents.at(i).energy;

			    if( channel_ < 4) // check gamma energy qualification
			      {if(eCheck < 344 && eCheck > 166 )
				  naiOk_ = true;}
			    else
			      if(eCheck < 123.7 && eCheck > 28.6)
				naiOk_ = true;
			    if(naiOk_)
			      groupNaiEvents[channel_] = vecNaiEvents.at(i);
				
			    if( (groupNaiEvents[0].time > 0 && groupNaiEvents[2].time > 0 && groupNaiEvents[1].time > 0 && groupNaiEvents[3].time > 0) 
				&& abs(groupNaiEvents[0].time - groupNaiEvents[2].time) <= 3 // almost at the same time;
				&& abs(groupNaiEvents[1].time - groupNaiEvents[3].time) <= 3 
				&& abs(groupNaiEvents[0].time - groupNaiEvents[1].time) <= 3 
				)
			      {
				double time_ = (groupNaiEvents[0].time > groupNaiEvents[2].time) ? groupNaiEvents[0].time : groupNaiEvents[2].time;
				double energy_ = (groupNaiEvents[0].energy + groupNaiEvents[1].energy + groupNaiEvents[2].energy + groupNaiEvents[3].energy)/2.;

				//---for test---
				/*
				cout << setprecision(6) << xEnergy << ", "
				     << setprecision(6) << energy_ << ", "
				     << setprecision(15) << time - time_ << "\n \n";	*/
			       

				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_ + 100); // 710
				plot(DD_DELTAT_NAIENERGY_IN, energy_, time - time_ + 100); // 711
				groupNaiEvents[0].Clear(); groupNaiEvents[2].Clear(); groupNaiEvents[3].Clear(); groupNaiEvents[1].Clear(); naiOk_ = false;
				break;
			      } // 0, 1, 2, 3

			    if( (groupNaiEvents[4].time > 0 && groupNaiEvents[6].time > 0)
				&& abs(groupNaiEvents[4].time - groupNaiEvents[6].time) <= 3 // almost at the same time;
				&& abs( (groupNaiEvents[4].energy - groupNaiEvents[6].energy)/groupNaiEvents[4].energy*100. < 3.5 ) // energy almost the same
				)
			      {
				double time_ = (groupNaiEvents[4].time > groupNaiEvents[6].time) ? groupNaiEvents[4].time : groupNaiEvents[6].time;
				double energy_ = (groupNaiEvents[4].time > groupNaiEvents[6].time) ? groupNaiEvents[4].energy : groupNaiEvents[6].energy;
				...
				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_ + 100); // 710
				plot(DD_DELTAT_NAIENERGY_OUT, energy_, time - time_ + 100); // 712
				groupNaiEvents[4].Clear(); groupNaiEvents[6].Clear(); naiOk_ = false;
				break;
			      } // 4, 6

			    if( (groupNaiEvents[5].time > 0 && groupNaiEvents[7].time > 0)
				&& abs(groupNaiEvents[5].time - groupNaiEvents[7].time) <= 3 // almost at the same time;
				&& abs( (groupNaiEvents[5].energy - groupNaiEvents[7].energy)/groupNaiEvents[5].energy*100. < 3.5 ) // energy almost the same
				)
			      {
				double time_ = (groupNaiEvents[5].time > groupNaiEvents[7].time) ? groupNaiEvents[5].time : groupNaiEvents[7].time;
				double energy_ = (groupNaiEvents[5].time > groupNaiEvents[7].time) ? groupNaiEvents[5].energy : groupNaiEvents[7].energy;
				...
				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_ + 100); // 710
				plot(DD_DELTAT_NAIENERGY_OUT, energy_, time - time_ + 100); // 712
				groupNaiEvents[5].Clear(); groupNaiEvents[7].Clear(); naiOk_ = false;
				break;
			      } // 5, 7
			  }// endif
		      } // for-loop: examining Nai events;
