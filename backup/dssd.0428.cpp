...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  */
  ...
  DeclareHistogram2D(9, 8000, 150, "DSSD-Ef vs. PIN-Ef"); // 709                                                                                                
  DeclareHistogram2D(10, 8000, 200, "DSSD-Ef vs. Dt"); // 710
  DeclareHistogram2D(11, 8000, 150, "DSSD-Ef vs. PIN-Ef, !MCP"); // 711
  DeclareHistogram2D(12, 8000, 200, "DSSD-Ef vs. Dt, !MCP");; // 712
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
	...
	if( abs((xEnergy - yEnergy)/xEnergy) < 0.035
	    ...
	    )
	  {
		...
		chain[0][x][y].time = time;
	      ...
	      else if(xEnergy < 8000 && yEnergy < 8000
  ...
  for(int i = 0; i < vecPinEventsAll.size(); i++){
	...
	plot(12, dssdE2, minTDiff + 10); // 712
      }    
    }
  }
