...
bool Dssd4JAEAProcessor::PreProcess(RawEvent &event) {
    ...
    //    const int xpos0 = 13; // for f16-3.ldf
    const int xpos0 = 16; // for f25-7.ldf
    const int ypos0 = 28;
    ...
    // output and plot
    if( (xVecRaw.size() + yVecRaw.size()) > 0){
      ofstream outfile;
      outfile.open("strip-upair.txt", std::ofstream::out | std::ofstream::app);
      // x-strip
      for(int ix = 0; ix < xVecRaw.size(); ix++){
	StripEvent str = xVecRaw.at(ix);
	outfile << std::setprecision(15) << str.t *Globals::get()->clockInSeconds() << "; "
		<< str.E << "; "
		<< str.pos << "; ";
	if(str.tr.empty()) outfile << "NO TRACE" << endl;
        else outfile << "HAS TRACE" << endl;
	Trace& xTrace = str.tr;
	for(vector<int>::iterator it = xTrace.begin();it != xTrace.end();it++){       // 715                                                              
	  plot(15,it-xTrace.begin(),xTraceNum,*it);
	}
	xTraceNum++;
	//	cout << "xTraceNum = " << xTraceNum << endl;
      }
      // y-strip
      for(int iy = 0; iy < yVecRaw.size(); iy++){
	StripEvent str = yVecRaw.at(iy);
	outfile << std::setprecision(15) << str.t * Globals::get()->clockInSeconds() << "; "
		<< str.E << "; "
		<< str.pos << "; ";
	if(str.tr.empty()) outfile << "NO TRACE" << endl;
	...
	for(vector<int>::iterator it = yTrace.begin();it != yTrace.end();it++){
	  plot(16,it-yTrace.begin(),yTraceNum,*it); // 716                                                               
	}
	yTraceNum++;
	//	cout << "yTraceNum = " << yTraceNum << endl;
      }
      outfile.close();
    }
    ...
    for(int ix = 0; ix < xyEventsTMatch_.size(); ix++){
      ...
      if( str1.pos == xpos0 || str2.pos == ypos0){      
      outfile2 << str1.E << "; "
	       << str2.E << "; "
	       << "(" << str1.pos << ", " << str2.pos << "); "
	       << std::setprecision(15) << str1.t << ", " 
	       << std::setprecision(15) << str2.t << endl;
      }// output aux strip events
    }
    outfile2.close();
