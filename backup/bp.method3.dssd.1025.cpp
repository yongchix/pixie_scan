	    ...
	    )
	  {
		...
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		     ...
		     && naiPair.GetCorrType().compare("nainai") == 0 // 511-pair is found
		      ...
		      if(vecNaiEvF.size() == naiPair.GetAuxNai())
			{
			  ...
			  plot(10, xEnergy, log(100 + time - naiPair.GetIndivTime(0)) ); // 710
			  plot(11, yEnergy, log(100 + time - naiPair.GetIndivTime(0)) ); // 711
