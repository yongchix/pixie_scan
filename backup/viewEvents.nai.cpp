...
{
  if(eventCntNai < 30)
    cout << "START TO PROCESS NAI RAW EVENT " << eventCntNai << endl;
  ...
  for (vector<ChanEvent*>::const_iterator it = naiEvents.begin();
    ...
    //  cout << (naiTime - 1.0414e11)/1e7 << endl; 
     // plot(DD_ENE_V_T,(naiTime - 1.0414e11)/1e8,calEnergy/5);
      ...
      if(eventCntNai == 26 || eventCntNai == 27)
	cout << number << ", "
	     << calEnergy << ", "
  ...
  if(eventCntNai < 30)
    cout << "NAI RAW EVENT " << eventCntNai << " PROCESSED! " << endl;
