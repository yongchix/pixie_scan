...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(13, 4000, 4000, "F/B-E consitency check"); // 713
  DeclareHistogram2D(14, 4000, 4000, "E-F vs. E-B"); // 714
  // --- //
  */

  // --- by Yongchi Xiao; 04/25/2016 --- //
  DeclareHistogram2D(9, 3000, 150, "DSSD-Ef vs. PIN-Ef"); // 709                                                                                                
  DeclareHistogram2D(10, 3000, 2000, "DSSD-Ef vs. TDiff.(PIN-f)"); // 710
  DeclareHistogram2D(11, 3000, 150, "DSSD-Eb vs. PIN-Eb"); // 711
  DeclareHistogram2D(12, 3000, 2000, "DSSD-Eb vs. TDiff.(PIN-b)");; // 712

  DeclareHistogram2D(13, 3000, 150, "prDSSD-Ef vs. PIN-Ef"); // 713                                                                                         
  DeclareHistogram2D(14, 3000, 2000, "prDSSD-Ef vs. TDiff.(PIN-f)"); // 714                                                                                     
  DeclareHistogram2D(15, 3000, 150, "prDSSD-Eb vs. PIN-Eb"); // 715                                                                                              
  DeclareHistogram2D(16, 3000, 2000, "prDSSD-Eb vs. TDiff.(PIN-b)");; // 716 
  //  DeclareHistogram2D(11, 3000, 150, "DSSD-Ef vs. PIN-Eb"); // 711
  //  DeclareHistogram2D(12, 3000, 200, "DSSD-Ef vs. TDiff.(PIN-b)"); // 712
  // --- //

  // --- by Yongchi Xiao; 03/09/2016 --- //
  //  DeclareHistogram2D(15, 1500, 512, "Specific traces-F"); // 715
  //  DeclareHistogram2D(16, 1500, 512, "Specific traces-B"); // 716
  // --- //

  // --- by Yongchi Xiao; 03/25/2016 --- //
  //  DeclareHistogram2D(15, 120, 18000, "Dssd-Sorted-All"); // 715
  // --- //
...
bool Dssd4JAEAProcessor::PreProcess(RawEvent &event) {
    ...
    // by Yongchi Xiao; 03/31/2016
        vector<ChanEvent*> pinEvents = event.GetSummary("pin:pin", true)->GetList();
    // by Yongchi Xiao; 04/12/2016
    //    vector<ChanEvent*> &pinEvents = sumMap["pin"]->GetList();
        
    // --- by Yongchi Xiao; 03/10/2016 --- //
    //    static int xTraceNum, yTraceNum; // for spec. 715/716
    // 03/25/2016
    static int eventNum715; // for spec.715
    // --- //
    ...
    // --- by Yongchi Xiao; 04/07/2016 --- //
    /*    
    ofstream outfileMax;
    outfileMax.open("e-max-pair.txt", std::iostream::out | std::iostream::app);
    //    outfile << "=== RAW EVENT ===" << endl;
    for(int ii = 0; ii < xyEventsEMatch_.size(); ii++){
      StripEvent s1 = xyEventsEMatch_.at(ii).first;
      StripEvent s2 = xyEventsEMatch_.at(ii).second;
      
      outfileMax << std::setprecision(15) << s1.t*Globals::get()->clockInSeconds() << "  " 
		 << s1.pos << "  " << std::setprecision(5) << s1.E << "  " << "dssd-front" << endl;
      outfileMax << std::setprecision(15) << s2.t*Globals::get()->clockInSeconds() << "  "
	      << s2.pos << "  " << std::setprecision(5) << s2.E << "  " << "dssd-back" << endl;
    ...
    */
    ...
    // --- by Yongchi Xiao; 03/04/2016 --- //
    // 03/10/2016
    // 03/20/2016
    // 03/25/2016
    
    // output strip events from x/yEventsTMatch
    //    const int ypos0 = 28;
    //    const int xpos0 = 13; const double t_imp = 6230.58891100; const double t_dt = 6230.86063000; // for f16
    //      const int xpos0 = 16;  const double t_imp = 8082.84400380; const double t_dt = 8082.91650000; // for f25
    //        const int xpos00 = 13; // for alpha-test
    
    // 04/06/2016
    //    const int xpos0[5] = {16, 12, 8, 20, 24}; // for f16
    //        const int xpos0[5] = {13, 9, 5, 17, 21}; // for f25 
    //    const int xpos0[5] = {16, 13, 10, 19, 22}; // for alpha-source, both f16 && f25
    //    const int ypos0[5] = {28, 24, 20, 22, 36}; 


    // transfer info. to new vector
    vector<StripEvent> xVecRaw, yVecRaw;
    for (vector< pair<StripEvent, bool> >::iterator itx = xEventsTMatch.begin();
         itx != xEventsTMatch.end();
         ++itx){
      StripEvent str = (*itx).first;
      //  if(str.pos == xpos0){ // specific xpos
      //            if( abs(str.pos - xpos0) < 2){ 
           if(1){
      //        if(str.pos == xpos0 || str.pos == xpos00){
	xVecRaw.push_back(str);
      }
      /*
    ...
    for (vector< pair<StripEvent, bool> > ::iterator ity = yEventsTMatch.begin();
         ity != yEventsTMatch.end();
         ++ity){
      StripEvent str = (*ity).first;
      // if(str.pos == ypos0){ // specific ypos
      //                  if( abs(str.pos - ypos0) < 2){
            if(1){
	yVecRaw.push_back(str);
      }
      /*
      for(int ii = 0; ii < 5; ii++){
	if(str.pos == ypos0[ii]){
    ...
    // --- by Yongchi Xiao; 04/04/2016
    // output front/back events seperately
    /*    
    ofstream outfile;
    if(yVecRaw.size() + xVecRaw.size() > 0){
      outfile.open("DSSD-dssd-pin.txt", std::iostream::out | std::iostream::app);
      StripEvent str;
      for(int i = 0; i < xVecRaw.size(); i++){
	str = xVecRaw.at(i);
	//		if(str.t * Globals::get()->clockInSeconds() > t_imp && str.t * Globals::get()->clockInSeconds() < t_dt){
		if(1){
	  outfile << std::setprecision(15) << str.t*Globals::get()->clockInSeconds() << "  " 
		  << str.pos << "  " 
		  << std::setprecision(7) << str.E << "  " 
		  << "dssd-front" << endl;
	}
      }// front
      for(int i = 0; i < yVecRaw.size(); i++){
	str = yVecRaw.at(i);
	//	if(str.t * Globals::get()->clockInSeconds() > t_imp && str.t * Globals::get()->clockInSeconds() < t_dt){
		if(1){
          outfile << std::setprecision(15) << str.t*Globals::get()->clockInSeconds() <<"  "
    ...
    */
    ...
    // --- 03/31/2016, use new vector of SimpleEvent
    vector<SimpleEvent> vecPINEv;
    for (vector<ChanEvent*>::iterator ip = pinEvents.begin();
	 ip != pinEvents.end(); ip++) {
      SimpleEvent se;
      if(1){
	se.AssignValue((*ip)->GetTime(), 
		       (*ip)->GetCalEnergy(), 
		       (*ip)->GetChanID().GetLocation(), 
		       "pin"); 
      }
      vecPINEv.push_back(se);
      int minTDiff = 1000;
      double dssdE = -1;
      int minTDiff2 = 1000;
      double dssdE2 = -1;
      if(se.channel > 0){ // front pin, PIN1
	for(int i = 0; i < xVecRaw.size(); i++){ // compared with raw signals
	  int tdiff_ = se.time - xVecRaw.at(i).t;
	  double dssdE_ = xVecRaw.at(i).E;
	  if(tdiff_ < minTDiff && tdiff_ > 0){
	    minTDiff = tdiff_;
	    dssdE = dssdE_;
	  }
	}if(dssdE > 0){
	  plot(9, dssdE, se.energy); // 709                                                                                                                     
	  plot(10, dssdE, 10 + minTDiff); // 710                                                                                                              
	}
	for(vector< pair<StripEvent, StripEvent> >::iterator it = xyEventsTMatch_.begin();
	  ...
	  plot(13, dssdE, se.energy); // 713                                                                                                                     
          plot(14, dssdE, 10 + minTDiff); // 714
	}
      }
      
      if(se.channel == 0){ // back pin, PIN0
	//	continue;
	for(int i = 0; i < yVecRaw.size(); i++){
	  int tdiff_ = se.time - yVecRaw.at(i).t;
	  double dssdE_ = yVecRaw.at(i).E;
	  if(tdiff_ < minTDiff && tdiff_ > 0){
	    minTDiff = tdiff_;
	    dssdE = dssdE_;
	  }
	...
	if(dssdE > 0){
	  plot(11, dssdE, se.energy); // 711
	  plot(12, dssdE, 10 + minTDiff); // 712
	}
	for(vector< pair<StripEvent, StripEvent> >::iterator it = xyEventsTMatch_.begin();
            it != xyEventsTMatch_.end(); ++it){ // compared with paired signals                                                                                  
          double yEnergy = (*it).second.E;
          int tdiff_ = se.time - (*it).second.t;
          if(tdiff_ < minTDiff2 && tdiff_ > 0){
            minTDiff2 = tdiff_;
            dssdE2 = yEnergy;
          }
        }if(dssdE2 > 0){
          plot(15, dssdE, se.energy); // 715                                                                                                                     
          plot(16, dssdE, 10 + minTDiff); // 716                                                                                                                 
        }
      }
    ...
    /*
    outfile.open("DSSD-dssd-pin.txt", std::iostream::out | std::iostream::app);
    for(int i = 0; i < vecPINEv.size(); i++){
      SimpleEvent se = vecPINEv.at(i);
      outfile << std::setprecision(15) << se.time << "  " 
	      << se.channel << "  " 
	      << std::setprecision(7) << se.energy << "  ";
      if(se.channel == 0) outfile << "pin-b" << endl;
      else outfile << "pin-f" << endl;
    }
    outfile.close();
    */
    ...
    /*
    // --- by Yongchi Xiao; 03/27/2016 --- //                                                                                                 
    // output MWPC-DSSD correspondence
    vector<ChanEvent*> mwpcEvents =    event.GetSummary("mcp", true)->GetList(); // get # of MWPC events;
    int mult_mwpc = event.GetSummary("mcp", true)->GetMult();
    int mult_dssd_all = vecAllRaw.size();
    int mult_dssd_front = vecXAllRaw.size();
    int mult_dssd_back = vecYAllRaw.size();
    ofstream ofile;
    ofile.open("check-mwpc-dssd.txt", std::iostream::out | std::iostream::app);
    double time_check = vecAllRaw.at(0).first.t;
    if(time_check * Globals::get()->clockInSeconds() > t_imp
       && time_check * Globals::get()->clockInSeconds() < t_dt
       ){
      if(mult_mwpc > 0)  ofile << "HAS_MWPC  ";
      else ofile << "NULL  ";
      ofile << mult_dssd_all << "  " << mult_dssd_front << "  " << mult_dssd_back << endl;
    }ofile.close();
    // --- //
    */  
    ...
    // --- 03/19/2016 --- //
    // --- 03/20/2016 --- // 	
    // output paired events only
    /*
    ofstream outfile2;
    outfile2.open("strip-pair.txt", std::ofstream::out | std::ofstream::app); 
    for(int ix = 0; ix < xyEventsTMatch_.size(); ix++){
      StripEvent str1 = xyEventsTMatch_.at(ix).first;
      StripEvent str2 = xyEventsTMatch_.at(ix).second;
      if( //(str1.pos == xpos0 || str2.pos == ypos0)  // specific strips
	 ( abs(str1.pos - xpos0) < 2 || abs(str2.pos - ypos0) < 2) // adjacent strips
	  && str1.t * Globals::get()->clockInSeconds() > t_imp
	  && str1.t * Globals::get()->clockInSeconds() < t_dt ){
	outfile2 << std::setprecision(15) << str1.t*Globals::get()->clockInSeconds() << "  "
		 << std::setprecision(15) << str2.t*Globals::get()->clockInSeconds() << "  "
		 << " " << str1.pos << "  " << str2.pos << "  "
		 << std::setprecision(7) << str1.E << "  " 
		 << std::setprecision(7) << str2.E << endl;
      }// limit events in a given timing range.
    }
    outfile2.close();
    */  
    // --- //
...
static PixelEvent preHit; // by Yongchi Xiao; 02/15/2016
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  vector<SimpleEvent> vecPinEventsAll;
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	// --- by Yongchi Xiao; 03/09/2016 --- //
	// locate buffer # corresponding to preceding 113Ba implant
	//	if(time == 623058891106){
	/*
	if(time == 808284400384){
	  cout << "\n\n\n>>> 113Ba implant caught! <<<\n\n\n"
	       << "E: " << xEnergy << ", " << "yEnergy" 
	       << endl;
	}
	*/
	...
	// --- output info. for specific pixels --- //
	// by Yongchi Xiao; 02/15/2016
	...
	// for new data
	// to output strip events; mods on 02/29/2016
	//	int x0 = 25, y0 = 1;
	//	int x0 = 27, y0 = 12;
	//	int x0 = 24, y0 = 12;
	/*
	if(x == x0 || y == y0){
	  ofstream outfile;
	  outfile.open("strip-ev.txt", std::ofstream::out | std::ofstream::app);
	  if(preHit.time < 0){ // if preceding hit not found
	    // assign value
	    preHit.energyF = xEnergy;
	    preHit.energyB = yEnergy;
	    preHit.time = time;
	    // output
	    outfile << time << "  pos:("
		    << x << ",  "
	  ...
	  else{ // if preceding hit found
	    double delta = time - preHit.time;
	    outfile << delta << "  pos:("
		    << x << ",  "
		    << y << ");   E:("
		    << xEnergy << ",  "
	...
	*/
	...
	// --- fill 713 to see front/back consistency --- //
	// by Yongchi Xiap; 03/02/2016
	/*
	plot(13, abs(xEnergy - yEnergy)/4., yEnergy/4.); // 713
	plot(14, xEnergy/4., yEnergy/4.); // 714
	*/
	// --- //
	...
	if(isDecay){	  
	  if(chain[1][x][y].time == -1){ // 1st component not found yet
	    ...
	    for(int ix = 0; ix < vecNaiEventsAll.size(); ix++){
	      SimpleEvent se = vecNaiEventsAll.at(ix);
	      chain[1][x][y].vecNaiInfo.push_back(make_pair(se.channel, se.energy)); 
	    }
	  }	  	  
	    ...
	    if(time - chain[1][x][y].time < (9350*5*2) && // 5 times of T1/2 of 109I: 93.5us
	      ...
	      for(int ix = 0; ix < vecNaiEventsAll.size(); ix++){
		SimpleEvent se = vecNaiEventsAll.at(ix);
		chain[2][x][y].vecNaiInfo.push_back(make_pair(se.channel, se.energy)); 
	      }
	      // give a flag
	      longChainFound = true;
	    }
	...
	if(canFillDT)
	  {
	    ...
	    */
	    ...
	    /*
	    cout << " ----- RAW CHAN ENERGY ---- " << endl;
	    cout << xEnergy << ", " << yEnergy << endl
	    	 << endl;
	    */
	    ...
	    // 02/19/2016
	    /*
	    cout << "\n has " << vecNaiEventsAll.size() << " gamma-ray hits...\n";
	    ofstream outfile;
	    outfile.open("dt-gamma.txt", std::ofstream::out | std::ofstream::app);
	    for(int ix = 0; ix < vecNaiEventsAll.size(); ix ++){
	      SimpleEv se = vecNaiEventsAll.at(ix);
	      outfile << se.GetTime() - time << "  " 
		      << se.GetChannel() << " "
		      << se.GetEnergy() << endl;
	    }outfile << "Next trace..." << endl;
	    outfile.close();
	    */
