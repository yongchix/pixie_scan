...
bool NaIProcessor::Process(RawEvent &event)
{
  ...
  for (vector<ChanEvent*>::const_iterator it = naiEvents.begin();
      ...
      if(number<=3)
	{
      ...
      if(sumSignal == 1 ) // if a interesting summed signal is found;
	{
	  if( (calEnergy < 5000) && (calEnergy > 1) ) // receive a NaI signal;
		...
		if(cnt > 0 && 
		   naiev.time - groupNaiEvents[cnt-1].time <= 3 // make sure these two signals correspond to the same event; interval <= 30ns
		   ) 
		  {groupNaiEvents[cnt] = naiev; cnt++;}
		else
		  {canFill = true; cnt = 0;}
