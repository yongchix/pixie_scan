...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(10, 4028, 1024*5, "Time interval vs. DSSD - E"); // 710
  DeclareHistogram2D(11, 1024, 1024, "NaI-E1 vs. NaI-E2"); // 711
	    ...
	    )
	  {
		...
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		     ...
		     && 100 + time - naiPair.GetIndivTime(0) < 5000
		      ...
		      if(vecNaiEvF.size() == naiPair.GetAuxNai())
			{
			  ...
			  plot(11, naiPair.GetIndivEnergy(0), naiPair.GetIndivEnergy(1)); // 711
			  naiPair.Clear();
			}
