...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  // -- by Yongchi Xiao; 04/21/2016 --- //
  // asked by Riccardo to give a spectrum gate/anti-gate on 511 keV photons
  /*
  DeclareHistogram2D(9, decayEnergyBins, xBins, "Decays-F no condition"); // 709
  DeclareHistogram2D(10, decayEnergyBins, xBins, "Decays-B no condition"); // 710
  DeclareHistogram2D(11, decayEnergyBins, xBins, "Decays-F gated on NaI"); // 711
  DeclareHistogram2D(12, decayEnergyBins, xBins, "Decays-B gated on NaI"); // 712
  DeclareHistogram2D(13, decayEnergyBins, xBins, "Decays-F gated on 511-photons"); // 713
  DeclareHistogram2D(14, decayEnergyBins, xBins, "Decays-B gated on 511-photons"); // 714
  DeclareHistogram2D(15, decayEnergyBins, xBins, "Decays-B gated on paired 511-photon"); // 715
  DeclareHistogram2D(16, decayEnergyBins, xBins, "Decays-B gated on paired 511-photon"); // 716
  */
  // --- //

  // --- by Yongchi Xiao; 04/25/2016
  DeclareHistogram2D(9, 3000, 250, "DSSD-Ef vs. PIN-Ef"); // 709
  DeclareHistogram2D(10, 3000, 200, "DSSD-Ef vs. TDiff."); // 710
  // --- //
...
bool Dssd4JAEAProcessor::PreProcess(RawEvent &event) {
    ...
    */
    ...
    for (vector<ChanEvent*>::iterator ip = pinEvents.begin();
	 ip != pinEvents.end(); ip++){
      SimpleEvent se;
      if((*ip)->GetChanID().GetLocation() > 0){ // channel = 1 or 2;
	se.AssignValue((*ip)->GetTime(), 
      ...
      /*
	outfile << std::setprecision(15) << se.time*Globals::get()->clockInSeconds() << "  "
	<< se.channel << "  "
	...
	if(tdiff_ < minTDiff && tdiff_ > 0){
      ...
      if(dssdEf > 0){
	plot(9, dssdEf, se.energy); // 709
	plot(10, dssdEf, minTDiff); // 710
      }
    }//PIN
      //    outfile.close();
    // --- //
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	// no matter whether it is a decay or not
	if(pinEvents.size()>0){
	...
	if(isDecay){
	  ...
	  if(naiPair.GetCorrType().compare("nainai") == 0){  // gated on paired 511-photons
	    plot(15, xEnergy, x);
	    plot(16, yEnergy, y);
	  }
	  */
