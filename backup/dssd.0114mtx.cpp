...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(22, 3500, 1500, "log(TDiff) vs. 1st DSSD-E"); // 722
  DeclareHistogram2D(23, 3500, 1500, "log(TDiff) vs. 2nd DSSD-E"); // 723 
  // --- //
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(decayPairFound){
	  ...
	  plot(22, chain[1][x][y].energyF, 100*log(tdiff)); // 722
	  plot(23, chain[2][x][y].energyB, 100*log(tdiff)); // 723
