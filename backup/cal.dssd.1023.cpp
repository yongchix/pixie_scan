...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(8, 1024, 50, "DSSD pos-F vs. DSSD E-F"); // 708
  DeclareHistogram2D(9, 1024*2, 50, "DSSD pos-B vs. DSSD E-B"); // 709
	    ...
	    )
	  {
		...
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event
			...
			if(time - chain[1][x][y].GetTime() < (500*100) // time interval < 500 us
			   && (time - chain[1][x][y].GetTime() > (100*100))
			   ) // time interval > 100 us
			  {
			    plot(7, chain[1][x][y].GetEnergy(), xEnergy); // 707
			    //			    plot(8, xEnergy, x); // 708
			    //			    plot(9, yEnergy, y); // 709
			    chain[1][x][y].Clear();
			  }
			else
