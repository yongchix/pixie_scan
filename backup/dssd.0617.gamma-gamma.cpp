...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
	...
	DeclareHistogram2D(15, 3000, 5000, "DSSD-EF vs. Dt, 40 us"); // 715
	DeclareHistogram2D(16, 3000, 8000, "DSSD-EF vs. Dt, 80 us"); // 716
	DeclareHistogram2D(17, 3000, 8000, "DSSD-EF vs. Dt, 100 us"); // 717
				...
				if(isDecay) {	  
					...
					if(naiPair.CheckCorr()) {
						if( (time - naiPair.GetTime()) > 0
							...
							if( (time - naiPair.GetTime())*Globals::get()->clockInSeconds() < 0.5*gammaProtonWin_ 
								&& (time - chain[0][x][y].time)*Globals::get()->clockInSeconds() > 0.5*gammaProtonWin_) {
								plot(15, xEnergy, time - naiPair.GetTime()); // 715
							}
							if( (time - naiPair.GetTime())*Globals::get()->clockInSeconds() < gammaProtonWin_
								&& (time - chain[0][x][y].time)*Globals::get()->clockInSeconds() > gammaProtonWin_) {
								plot(16, xEnergy, time - naiPair.GetTime()); // 716
							}
							if( (time - naiPair.GetTime())*Globals::get()->clockInSeconds() < 1.25*gammaProtonWin_
								&& (time - chain[0][x][y].time)*Globals::get()->clockInSeconds() > 1.25*gammaProtonWin_) {
								plot(17, xEnergy, 0.5*(time - naiPair.GetTime())); // 717
								if( abs(xEnergy - 300) < 200 )
