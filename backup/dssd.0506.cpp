...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  */
  ...
  // delay correlations
  DeclareHistogram2D(9, 3000, 150, "delayed DSSD-Ef vs. PIN-Ef"); // 709                                                                                                
  DeclareHistogram2D(10, 3000, 400, "delayed DSSD-Ef vs. Dt"); // 710
  // prompt correlations
  DeclareHistogram2D(11, 3000, 150, "All DSSD-Ef vs. PIN-Ef, !MCP"); // 711
  DeclareHistogram2D(12, 3000, 400, "All DSSD-Ef vs. Dt, !MCP");; // 712

  DeclareHistogram2D(13, 3000, 50, "DSSD-decays-F"); // 713                                                                                         
  DeclareHistogram2D(14, 3000, 50, "DSSD-decays-B"); // 714                                                                                     
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector<ChanEvent*>::const_iterator it = naiEvents.begin(); it != naiEvents.end(); it++) 
    {
      ...
      if( (number < 4 && calEnergy > 125 ) || (number > 3 && calEnergy > 150) ){
	SimpleEvent naiev; 
  ...
  for(int i = 0; i < vecPinEventsAll.size(); i++){
	...
	plot(9, dssdE, se.energy); // 709
	plot(10, dssdE, minTDiff + 10); // 710
      }
    */
      for(vector< pair<SimpleEvent, SimpleEvent> >::iterator ix = vecDecays.begin(); 
	  ix != vecDecays.end();
	  ix++){
	double xEnergy_ = ix->first.energy;
	int tdiff_ = se.time - ix->first.time;
        double dssdT_ = ix->first.time;
	...
	plot(11, dssdE2, se.energy); // 711
	plot(12, dssdE2, minTDiff2 + 10); // 712
	outfile << std::setprecision(15) << dssdT2 << "  " << std::setprecision(5) << dssdE2 << "  " 
		<< std::setprecision(15) << se.time << "  " << std::setprecision(5) << se.energy << "  "
		<< minTDiff2 << endl;
	if(minTDiff2 > 5){
	  bool qualified = true;
	  for(vector<SimpleEvent>::iterator ne = vecNaiEventsAll.begin();
	      ne != vecNaiEventsAll.end();
	      ne++){
	    if( abs(ne->time - dssdT2) < 15 ){
	      qualified = false;
	      break;
	    }
	  }
	  if(qualified == true){
	    plot(9, dssdE2, se.energy); // 709
	    plot(10, dssdE2, minTDiff2 + 10); // 710
	  }
	}
      }    
    }outfile.close();
