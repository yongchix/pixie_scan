...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(10, 4028, 1024, "log(dT) vs. DSSD - EF"); // 710
	    ...
	    )
	  {
		...
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		    ...
		    {
		      vector<SimpleEv> vecNaiEvF;
		      for(int i = 0; i < vecNaiEventsAll.size(); i++)
			{
			  SimpleEv se = vecNaiEventsAll.at(i);
			  if(100 + time - se.GetTime() < 75) // these nai-hits are really following the dssd-hits
			    vecNaiEvF.push_back(se);
			}
		      if(vecNaiEvF.size() == naiPair.GetAuxNai())
			{
			  ...
			  plot(10, xEnergy, 100*log(100 + time - naiPair.GetIndivTime(0)) ); // 710
			  plot(11, yEnergy, 100*log(100 + time - naiPair.GetIndivTime(0)) ); // 711
			  plot(12, naiPair.GetIndivEnergy(0), naiPair.GetIndivEnergy(1)); // 712

			  if(naiPair.GetIndivChannel(2) == 0)// pin1
			    plot(253, naiPair.GetIndivEnergy(2)); // 953			    
			  else if(naiPair.GetIndivChannel(2) == 2) // pin2
			    plot(254, naiPair.GetIndivEnergy(2)); // 954
