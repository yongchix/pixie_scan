	    ...
	    )
	  {
		...
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		     ...
		     && 100 + time - naiPair.GetTime() > 0
		     && 100 + time - naiPair.GetTime() < 4000
		     && naiPair.GetTime() - 50 > chain[0][x][y].time // no implantation after 511-pair
		    )
		    {
		      plot(10, xEnergy, 100 + time - naiPair.GetTime()); // 710
		      naiPair.Clear();
