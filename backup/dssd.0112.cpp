...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(11, 1000, 1000, "NaI(E) vs. DSSD(calb_trace2F), plug-in"); // 711, 01/06/2016
  DeclareHistogram2D(12, 1000, 1000, "NaI(E) vs. DSSD(calib_trace2F, barrel"); // 712, 01/07/2016
  ...
  DeclareHistogram2D(15, 1000, 1000, "DSSD-EF(tr1) vs. DSSD-EF(tr2)"); // 715, 01/12/2016
	...
	if(canFillDT)
	  {
		...
		plot(12, calib_trace_energy2F, se.GetEnergy()); // 712, 01/07/2016
	      */
	      if(!hasMcp || (hasMcp && abs(mwpcTime - time) >= 50) ){ // ok
