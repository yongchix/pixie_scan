...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(15, 3000, 5000, "DSSD-F vs. Dt"); // 715
  DeclareHistogram1D(16, 3500, "DSSD-EF - test"); // 716  
  // --- //
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(isDecay){	  
	  ...
	  /* --- below is for 511keV-gammas vs. proton correlation --- */
	  if(naiPair.CheckCorr()){
	    plot(16, xEnergy); // 716
	    if( (time - naiPair.GetTime())*Globals::get()->clockInSeconds() < 0.00003 // 30 us
		&& (time - chain[0][x][y].time)*Globals::get()->clockInSeconds() > 0.00003 // 30 us
		&& (time - naiPair.GetTime()) > 0
		&& !hasNaI
		){
	      plot(15, xEnergy, time - naiPair.GetTime()); // 715
	      naiPair.Clear();
	    }else naiPair.Clear();
	  }


	  /* -- below is for correlation matrix --- */
	  /*
	  PixelEvent pe;
	  ...
	  if(chain[1][x][y].time == -1){ // 1st component not found yet                                                                                          
	    ...
	    for(vector<SimpleEvent>::iterator inev = vecNaiEventsAll.begin();
		inev != vecNaiEventsAll.end();
		inev++){
	      chain[1][x][y].vecNaiInfo.push_back(make_pair(inev->channel, inev->energy));
	    }
          }
	      ...
	      if( 
		 pDecay.second.time > 0 && pDecay.second.time - pDecay.first.time > 3
		 //		 1
	       ){
		...
		for(vector<SimpleEvent>::iterator inev = vecNaiEventsAll.begin();
		    inev != vecNaiEventsAll.end();
		    inev++){
		  chain[2][x][y].vecNaiInfo.push_back(make_pair(inev->channel, inev->energy));
		}
		...
		//		for(vector< pair<int, double> >::iterator it = pDecay.first.vecNaiInfo.begin();
		//		    it != pDecay.first.vecNaiInfo.end();
		//		    it++){
		//		  outfile << it->first << "  " 
		//			  << it->second << "  ";
		//		}outfile << endl;
