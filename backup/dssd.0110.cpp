...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(11, 6000, 1000, "NaI(E) vs. DSSD(calb_trace2F), hasMcp"); // 711, 01/06/2016
  DeclareHistogram2D(12, 6000, 1000, "NaI(E) vs. DSSD(calib_trace2F, noMCP"); // 712, 01/07/2016
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(hasNaI){
	  for(int i = 0; i < vecNaiEventsAll.size(); i++){
	    ...
	    plot(13, se.GetTime() - time + 1000); // 713, 01/07/2016
	  }
	}

	if(hasPin){
	  for(int i = 0; i < vecPinEventsAll.size(); i++){
	    SimpleEv se = vecPinEventsAll.at(i);
	    plot(14, se.GetTime() - time + 1000); // 714, 01/07/2016
	  }
	}	    
	*/
	// --- //
	    ...
	    )
	  {
		...
		if(xEnergy > (30000/4.00)) // changed by Yongchi Xiao; 11/25/2015
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		     ...
		     && (time - chain[0][x][y].time)*pow(10, -2) > 400 // no ion in time window = 400us; modified by Yongchi Xiao; 11/25/2015
		     && (
		     ...
		     // verify correlation type
		     && naiPair.GetCorrType().compare("nainaipin") == 0 // 511-pair is found
		     
		     // timing gate
		     && 100 + time - naiPair.GetIndivTime(0) > 75
		     && 100 + time - naiPair.GetIndivTime(0) < 100000
		     && naiPair.GetIndivTime(0) - 25 > chain[0][x][y].time // no implantation after 511-pair

		     // require no gamma coming together
		     && !hasNaI // 11/23/2015
		     && naiPair.GetAuxNai() == 0 // by Yongchi Xiao; 11/24/2015
		    )
		    {
		      SimpleEv dssdX, dssdY;
		      string type = "dssd";
		      dssdX.AssignValue(time, xEnergy, x, type);
		      dssdY.AssignValue(time, yEnergy, y, type);
		      naiPair.AddEvent(dssdX);
		      naiPair.AddEvent(dssdY);
		      plot(10, xEnergy, (100 + time - naiPair.GetIndivTime(0)) ); // 710
		      plot(11, yEnergy, (100 + time - naiPair.GetIndivTime(0)) ); // 711
		      plot(12, naiPair.GetIndivEnergy(0), naiPair.GetIndivEnergy(1)); // 712
		      
		      ofstream outfile; // by Yongchi Xiao; 11/15/2015
		      outfile.open("counts710.txt", std::ofstream::out | std::ofstream::app);
		      outfile << naiPair.GetCorrType() 
			      << setw(10) << xEnergy
			      << setw(3) << x << setw(3) << y
			      << setw(10) << naiPair.GetIndivEnergy(0)
			      << setw(10) << naiPair.GetIndivEnergy(1) 
			      << setw(3) << naiPair.GetIndivChannel(0)
			      << setw(3) << naiPair.GetIndivChannel(1)
			      << setw(8) << 100 + time - naiPair.GetIndivTime(0)
			      << "   "
			      << setw(10) << naiPair.GetIndivEnergy(2) 
			      << setw(3) << naiPair.GetIndivChannel(2)
			      << "   "
			      << (time - chain[0][x][y].time)
			      << "\n"; 
		      outfile.close();
	...
	*/
	...
	if( calib_trace_energy1F > 0 && calib_trace_energy1B > 0 &&
	    ...
	    calib_trace_energy1F > 800 && 
	    calib_trace_energy2F > 800
	    )
	  hasBothSide2 = true; // by Yongchi Xiao; 01/05/2016
	// --- by Yongchi Xiao; 01/06/2016 add cut for trace energy --- //

	if( //abs(calib_trace_energy1F - calib_trace_energy1B)/calib_trace_energy1F < 0.05 &&
	    //abs(calib_trace_energy2F - calib_trace_energy2B)/calib_trace_energy2B < 0.05 &&
	    hasBothSide2 
	...
	if(canFillDT)
	  {
	    ...
	    // by Yongchi Xiao
	    //if(vecNaiEventsAll.size() > 0)
	    //  cout << "\n\n\n got at least a hit \n\n\n";
	    for(int i = 0; i < vecNaiEventsAll.size(); i++){
	      SimpleEv se = vecNaiEventsAll.at(i);
	      plot(10, se.GetChannel(), se.GetEnergy()); // 710
	      if(hasMcp)
		plot(11, calib_trace_energy2F, se.GetEnergy()); // 711, 01/06/2016
	      else
		plot(12, calib_trace_energy2F, se.GetEnergy()); // 712, 01/07/2016
	    }
	    //-----//
	    ...
	    //--- see efficiency of MWPC ---//
	    /*
	    if(hasMcp){
	      cout << "\n hasMcp: time difference = " << mwpcTime - time << endl;
	      plot(12, mwpcTime - time + 1000); // 712, 01/06/2016
	    }
	    else cout << "\n no Mcp \n";
	    */
	    //---  by Yongchi Xiao 01/06/2015 ---//
