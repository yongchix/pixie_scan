...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  ofstream outfile; // $
  //outfile.open("dssd-nai-correlation.txt", ios::app); // $
	    ...
	    ) 
	  {
		...
		if(vecNaiEvents.at(i).time - time <=0 )
		...
		else
		  {
		    ...
		    if(nev.channel < 4)
		      {
			plot(D_INTVL_DSSD_NAI, (nev.time - time)+100); // 715
			plot(DD_INTVL_DSSD, (nev.time - time)+100, xEnergy);     // 716
			plot(DD_INTVL_NAI, (nev.time - time)+100, nev.energy);     // 717
		      }
		    break;
		  }
		
		//	outfile << setprecision(15) << nev.time << "; ch-" << setprecision(2) << nev.channel << endl;//$
	      }
	  }
	//------------------------------------------------------
      }
	...
	if( //xEnergy < 2730 && yEnergy < 2730
	    ...
	    )          
	  {
		...
		if(!hasNaI) // can it have gamma radiation? 
		  ...
		  && (time - chain[1][x][y].time)*pow(10, -8) < 0.0300
		  )
		{
		  ...
		  if( (energy1F - 880)*(energy1F -1010) < 0 
			...
			// yc 1947 2251
		    {
		      ...
		      /*
		      outfile << setprecision(15) << time << "; " << " <= DSSD SUMMED SIGNAL" << endl;//$
		      ...
		      for(int i = 0; i < vecNaiEvents.size(); i++)
			{
			  ...
			  plot(D_INTVL_DSSD_NAI, (time - nev.time)+100); // 715
			  plot(DD_INTVL_DSSD, (time - nev.time)+100, xEnergy);     // 716
			  plot(DD_INTVL_NAI, (time - nev.time)+100, nev.energy);     // 717
	...
	if(canFillDT)
	  {
	    ...
	    cout << "-----------NAI SIGNALS-----------" << endl;
	    cout << "there are " << vecNaiEvents.size() << " nai signals coming together; " << endl;
