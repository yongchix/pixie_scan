...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  //  DeclareHistogram1D(D_DELTAT_GG, 1024*8, "Time interval between gammas1, 2"); // 715
  //  DeclareHistogram2D(16, 1024, 1024, "Nai - E1(x) vs. Nai - E2(y)"); // 716
  //DeclareHistogram1D(17, 50, "# of correlations: 511-proton"); // 717
  //-----------------------------------------------------------------------------------------
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  //--------------- Nai Events ------------------------------------
  vector<NaiEvent> vecNaiEventsAll;
	...
	if( abs((xEnergy - yEnergy)/xEnergy) < 0.035
	    ...
	    )
	  {
	    ...
	    chain[0][x][y].time = time;
	      ...
	      {
		if(xEnergy > (30000/3.93)){ // if E > 30MeV
		  chain[0][x][y].energyF = xEnergy;
		  chain[0][x][y].energyB = yEnergy;
		  chain[0][x][y].time = time;
		} // taken as an implantation
		else{
		  NaiEvent groupNaiEventsP[8]; 
		  NaiEvent groupNaiEventsF[8];
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		     ...
		     )
		    {
		      ...
		      vector<NaiEvent> vecNaiEvents; // only used by this DSSD signal	      

		      for(int i = 0; i < vecNaiEventsAll.size(); i++)
			if(vecNaiEventsAll.at(i).time < time + 50)
			  vecNaiEvents.push_back(vecNaiEventsAll.at(i));
			
		      for(int i = 0; i < vecNaiEvents.size(); i++)
			{
			  // gate on both preceding gamma and following gamma
			  ...
			  if(naiOk_p)
			    groupNaiEventsP[channel_] = vecNaiEvents.at(i); // fill a possible 511keV photon
			  
			  // --------------------- handling any 511-pairs -------------------------------------------//
			  if( (groupNaiEventsP[0].time > 0 && groupNaiEventsP[5].time > 0) 
			      && abs(groupNaiEventsP[0].time - groupNaiEventsP[5].time) <= 3 
			      ) foundPairP[0] = true; 
			  
			  if( (groupNaiEventsP[1].time > 0 && groupNaiEventsP[6].time > 0)
			      && abs(groupNaiEventsP[1].time - groupNaiEventsP[6].time) <= 3 
			      ) foundPairP[1] = true;
			  
			  if( (groupNaiEventsP[2].time > 0 && groupNaiEventsP[7].time > 0)
			      && abs(groupNaiEventsP[2].time - groupNaiEventsP[7].time) <= 3 
			      ) foundPairP[2] = true;

			  if( (groupNaiEventsP[3].time > 0 && groupNaiEventsP[4].time > 0)
			      && abs(groupNaiEventsP[3].time - groupNaiEventsP[4].time) <= 3 
			      ) foundPairP[3] = true;
			  
			  if( (groupNaiEventsP[4].time > 0 && groupNaiEventsP[6].time > 0)
			      && abs(groupNaiEventsP[4].time - groupNaiEventsP[6].time) <= 3 // almost at the same time;
			  ...
			  if( (groupNaiEventsP[5].time > 0 && groupNaiEventsP[7].time > 0)
			      && abs(groupNaiEventsP[5].time - groupNaiEventsP[7].time) <= 3 // almost at the same time;
			  ...
			  if( foundPairP[0] == true)
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[0].time;
				energy_o = groupNaiEventsP[5].energy;
				energy_i = groupNaiEventsP[0].energy;

				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_p + 100); // 710
				plot(11, energy_i, time - time_p + 100); // 711, nai-in
				plot(12, energy_o, time - time_p + 100); // 712, nai-out
			  ...
			  if( foundPairP[1] == true)
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[1].time;
				energy_o = groupNaiEventsP[6].energy;
				energy_i = groupNaiEventsP[1].energy;

				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_p + 100); // 710
				plot(11, energy_i, time - time_p + 100); // 711, nai-in
				plot(12, energy_o, time - time_p + 100); // 712, nai-out

				groupNaiEventsP[6].Clear(); groupNaiEventsP[1].Clear();  foundPairP[1] = false;
			      }else{
				groupNaiEventsP[6].Clear(); groupNaiEventsP[1].Clear(); foundPairP[1] = false;
			      }
			    } // 1, 3
			  ...
			  if( foundPairP[2] == true)
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[2].time;
				energy_o = groupNaiEventsP[7].energy;
				energy_i = groupNaiEventsP[2].energy;
			  ...
			  if( foundPairP[3] == true)
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[3].time;
				energy_o = groupNaiEventsP[4].energy;
				energy_i = groupNaiEventsP[3].energy;
			  ...
			  if( foundPairP[4] == false )
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[4].time;
				energy_o = (groupNaiEventsP[4].time > groupNaiEventsP[6].time) ? groupNaiEventsP[4].energy : groupNaiEventsP[6].energy;

				plot(DD_DELTAT_DSSDENERGY, xEnergy, time - time_p + 100); // 710
				plot(DD_DELTAT_NAIENERGY_OUT, energy_o, time - time_p + 100); // 712
			  ...
			  if( foundPairP[5] == false)
			    {
			      if(i == vecNaiEvents.size() - 1){
				time_p = groupNaiEventsP[5].time;
				energy_o = (groupNaiEventsP[5].time > groupNaiEventsP[7].time) ? groupNaiEventsP[5].energy : groupNaiEventsP[7].energy;
