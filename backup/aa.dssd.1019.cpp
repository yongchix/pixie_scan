...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(7, 1024*4, 1024*4, "Correlation Matrix - F"); // 707
  DeclareHistogram2D(8, 1024*4, 1024, "Time diff. in correlation, offset=200"); // 708
  DeclareHistogram2D(9, 1024*4, 1024*2, "Time diff. between nai - DSSD 2nd, offset=200"); // 709
  DeclareHistogram2D(10, 1024*4, 512, "DSSD 2nd-E vs. NaI-E-in"); // 710
  DeclareHistogram2D(11, 1024*4, 512, "DSSD 2nd-E vs. NaI-E-out"); // 711
  //-----------------------------------------------------------------------------------------
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector<ChanEvent*>::const_iterator it = naiEvents.begin(); it != naiEvents.end(); it++) 
    {
      ...
      if(number < 4)
	{
	  if(calEnergy < 198.52 && calEnergy > 41.52)
	    vecNaiEvents.push_back(naiev);
	}else
	{
	  if(calEnergy < 359.87 && calEnergy > 30.31)
	    vecNaiEvents.push_back(naiev);
	}
    }
  ...
  static SimpleEv chain[2][40][40] = {}; // by Yongchi Xiao; 04/20/2015; 05/11/2015
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if( abs((xEnergy - yEnergy)/xEnergy) < 0.035
	  ...
	  {
	    if(hasMcp){
	      chain[0][x][y].AssignValue(time, xEnergy, x, DSSDtype);
	    } // an implantation
	    else
	      {
		if(xEnergy > (30000/3.93))
		  ...
		  if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event
		     && xEnergy > 100 && yEnergy > 100
		     && (
			 time != chain[0][x][y+1].GetTime()
			 && time != chain[0][x][y-1].GetTime()
			 && time != chain[0][x+1][y].GetTime()
			 && time != chain[0][x-1][y].GetTime()
			 && time != chain[0][x+1][y+1].GetTime()
			 && time != chain[0][x+1][y-1].GetTime()
			 && time != chain[0][x-1][y+1].GetTime()
			 && time != chain[0][x-1][y-1].GetTime()
		    ...
		    {
		      // refer to method1.0810.dssd.cpp
		      if(chain[1][x][y].GetTime() < 0)
			{
			  chain[1][x][y].AssignValue(time, xEnergy, x, DSSDtype);
			}
		      else // if 1st decay found
			if(time - chain[1][x][y].GetTime() < (30*1000*100))// resonable time interval: 30 ms
			  {
			    plot(7, chain[1][x][y].GetEnergy(), xEnergy); // 707

			    if(chain[1][x][y].GetEnergy() > 800 && chain[1][x][y].GetEnergy() < 1010
			       && xEnergy > 1974 && xEnergy < 2502
			       ) // in the zone of interest
			      // zone0: xc 800 1010; yc 1974 2502
			      {
				plot(8, chain[1][x][y].GetEnergy(), (time - chain[1][x][y].GetTime())*pow(10, -4) + 200 ); // 708
				if(hasNaI)
				  {
				    for(int i = 0; i < vecNaiEvents.size(); i++)
				      {
					plot(9, xEnergy, vecNaiEvents.at(i).GetTime() - time + 200); // 709
					if(vecNaiEvents.at(i).GetChannel() < 4)
					  plot(10, xEnergy, vecNaiEvents.at(i).GetEnergy()); // 710, in
					else
					  plot(11, xEnergy, vecNaiEvents.at(i).GetEnergy()); // 711, out
				      } 
				  }// if(hasNaI)
			      }// if (zone0)
			  ...
			  chain[1][x][y].AssignValue(time, xEnergy, x, DSSDtype); // too far away from 1st decay
