...
bool Dssd4JAEAProcessor::PreProcess(RawEvent &event) {
    ...
    */
    ...
    if(yVecRaw.size() + xVecRaw.size() > 0){
      outfile.open("dssd-events.txt", std::iostream::out | std::iostream::app);
      StripEvent str;
    ...
    */
    ...
    /*
    if(vecPINEv.size() > 0){
      outfile.open("pin-events2.txt", std::iostream::out | std::iostream::app);
      for(int i = 0; i < vecPINEv.size(); i++){
	SimpleEvent se = vecPINEv.at(i);
	//	if(se.time * Globals::get()->clockInSeconds() > t_imp && se.time * Globals::get()->clockInSeconds() < t_dt){
	if(1){
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for(int i = 0; i < vecNaiEventsAll.size(); i++){
    ...
    switch(channel_){
    case 0: if(energy_ < 149 && energy_ > 295){ found511 = true;} break;
    case 1: if(energy_ < 149 && energy_ > 295){ found511 = true;} break;
    case 2: if(energy_ < 149 && energy_ > 295){ found511 = true;} break;
    case 3: if(energy_ < 149 && energy_ > 295){ found511 = true;} break;
