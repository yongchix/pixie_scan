...
static CorrEv bapChain; // by Yongchi Xiao; 02/01/2016
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(isDecay){
	  ...
	  if(naiPair.GetCorrType().compare("nainaipin") == 0){
	    if(chain[1][x][y] == -1){
	      ...
	      if( abs(time - chain[1][x][y].time) < 9350*5 ){
	  ...
	  */
	  ...
	  if(chain[1][x][y].time == -1){ // 1st component not found yet
	    if(naiPair.GetCorrType().compare("nainaipin") == 0){ // make sure 1st component is gated on 511-pairs
	      ...
	      // fill the long chain	    
	      bapChain = naiPair;
	      bapChain.AddEvent(dssdX);
	      bapChain.AddEvent(dssdY);
	    }
	  }
	    ...
	    if(time - chain[1][x][y].time < (9350*5) && // 5 times of T1/2 of 109I: 93.5us
	      ...
	      // fill the long chain
	      bapChain.AddEvent(dssdX);
	      bapChain.AddEvent(dssdY); // bap chain completed!
	    }
	    else{ // too long time interval 
	      if(naiPair.GetCorrType().compare("nainaipin") == 0){
		// unless this decay is gated on 511-pair, it won't take over the existing 1st component
		chain[1][x][y].Clear(); // start a new pair
		chain[1][x][y].energyF = xEnergy;
		chain[1][x][y].energyB = yEnergy;
		chain[1][x][y].time = time;
		// re-organize the long chain
		bapChain.Clear();
		bapChain.AddEvent(dssdX);
		bapChain.AddEvent(dssdY);
	      }
	...
	if(longChainFound){
	  ...
	  outfile << bapChain.GetCorrType() << "  "; // correlation type
	  for(int ix = 0; ix < bapChain.GetCorrLength(); ix++){
	    outfile << bapChain.GetIndivType(ix) << "  " // type
		    << bapChain.GetIndivTime(ix) << "  " // time
		    << bapChain.GetIndivChannel(ix) << "  " // channel
		    << bapChain.GetIndivEnergy(ix) << "  " // energy
		    << endl;
	  }outfile.close();
