...
string fileName = "alpha-matrix.txt";
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if( abs((xEnergy - yEnergy)/xEnergy) < 0.035
	    ...
	    )
	  {
		...
		chain[0][x][y].time = time;
	      ...
	      else if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
	...
	// fill the decay chain
	if(isDecay){
	  if(chain[1][x][y].time == -1){
	    ...
	    if(time - chain[1][x][y].time < 3000000 &&
	       time - chain[1][x][y].time > 0 // in case the clock jump backwards;
	       // by Yongchi Xiao; 01/15/2016
	       ){
	      // if TDiff < 30 ms, taken as the 2nd component
	...
	if(decayPairFound){
	  plot(17, chain[1][x][y].energyF, chain[2][x][y].energyF); // 717

	  double tdiff = chain[2][x][y].time - chain[1][x][y].time; 
	  //	  cout << "\n TIME DIFF. " << tdiff << endl;
	  plot(22, chain[1][x][y].energyF, 10*pow(tdiff, 1/3.) ); // 722
	  plot(23, chain[2][x][y].energyF, 10*pow(tdiff, 1/3.) ); // 723

	  
	  
	  // no other choices, but have to output correlated events to
	  // re-construct a decay chain with all response from different
	  // detectors
	  // --- by Yongchi Xiao; 01/21/2016 --- //
	  ...
	  // handle correlation w/. NaI
	  for(int i = 0; i < vecNaiEventsAll.size(); i++){
	    SimpleEv se = vecNaiEventsAll.at(i);
	    if(se.GetChannel() < 4){// plug-in
	      plot(18, chain[1][x][y].energyF, se.GetEnergy()); // 718
	      plot(20, chain[2][x][y].energyF, se.GetEnergy()); // 720
	    }
	    else{
	      plot(19, chain[1][x][y].energyF, se.GetEnergy()); // 719
	      plot(21, chain[2][x][y].energyF, se.GetEnergy()); // 721
	    }
	    // whichever channel it came from
	    outfile << se.GetChannel() << "  "
		    << se.GetEnergy() << "  ";
	  }

	  if(vecNaiEventsAll.size() < 5){
	    for(int i = 0; i < (5 - vecNaiEventsAll.size()); i++){
	      outfile << "-1" << "  " // for channels
		      << "-2" << "  "; // for energies
	    }
	    outfile << endl;
	  }else outfile << endl;
	...
	if(canFillDT && hasNaI) // new condition added by Yongchi; 01/25/2016
	  {
	    // --- output all necessary info. into text file --- //
	    // by Yongchi Xiao; 01/25/2016
	    ofstream outfile;
	    outfile.open(fileName2.c_str(), std::iostream::out | std::iostream::app);
	    outfile << calib_trace_energy1F << "  "
		    << calib_trace_energy2F << "  "
		    << calib_trace_energy1B << "  "
		    << calib_trace_energy2B << "  "
		    << trace_time2F - trace_time1F << "  "
		    << trace_time2B - trace_time2F << "  ";
	    // ... to be continued...
	    // --- //
