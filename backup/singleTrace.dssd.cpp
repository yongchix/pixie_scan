...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  // 742 checked 
    ...
    // 774 
    DeclareHistogram2D(DD_ENERGY2F_ENERGY1F, SA, numTraces*8, "Single Trace - F"); // by Yongchi Xiao; for recording single trace; 08/26/2015
    // 775
    DeclareHistogram2D(DD_ENERGY2B_ENERGY1B, SA, numTraces*8, "Single Trace - B"); // by Yongchi Xiao; for recording single trace; 08/26/2015
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  static int traceNum;
  static int traceNumST; // by Yongchi Xiao; 08/26/2015
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	bool canFillST = false; // by Yongchi Xiao; 08/26/2015
	...
	if( //xEnergy < 2730 && yEnergy < 2730
	    ...
	    )          
	  {
		...
		if(chain[1][x][y].time < 0) // 1st blank
		  ...
		  if( (pxl.time - chain[1][x][y].time) >= 0  
		      && (pxl.time - chain[1][x][y].time)*pow(10, -8) < 0.0300 // interval < 30ms
		      ) 
		    {
		      chain[2][x][y] = pxl;
		      gotChain = true;
		...
		if( (chain[1][x][y].energyF - 800)*(chain[1][x][y].energyF - 1100) < 0 
		    && (chain[2][x][y].energyF - 1900)*(chain[2][x][y].energyF - 2600) < 0
		    ) // zoneXe;
		  //   xc 780 979
		  //   yc 830 1060
		  // zone0: xc 800 1100; yc 1900 2600
		  {
		    ofstream outfile;
		    outfile.open("findpixel.txt", ios::app); // by Yongchi Xiao; 08/20/2015
		    
		    outfile << setw(15) << chain[1][x][y].energyF
			    << setw(15) << chain[2][x][y].energyF
			    << "pixel: " << setw(5) << x << setw(5) << y << endl;
		    ...
		    plot(DD_INTVL_DSSD, naiPair[1].energy, naiPair[2].energy + 100);     // 716	  off set added for 2nd signal
		  }
		//-----------------------------------------------------------------------------------------------------------------//
		for(int i = 1; i < 3; i++)
		  {
		    chain[i][x][y].Clear(); naiPair[i].Clear();
		  }
		pxl.Clear(); 
		hasGoodGamma = false;
		gotChain = false;
		// initialization; by Yongchi Xiao; 08/19/2015
	      }
	  } 
	...
	// ---------------------------- HANDLE THE SINGLE TRACES -----------------------------------// by Yongchi Xiao; 08/26/2015
	if( xTrace.HasValue("filterEnergyCal") && yTrace.HasValue("filterEnergyCal")
	    && !xTrace.HasValue("filterEnergy2Cal") && !yTrace.HasValue("filterEnergy2Cal")
	    )
	  {
	    if( 
	       	(x == 15 && y == 15)
	    ...
	    canFillST = true;
	    // calib_trace_energy1F, calib_trace_energy1B
	  }
	if(canFillST == true)
	  {
	    for(vector<int>::iterator it = xTrace.begin(); it != xTrace.end(); it++) // 774
	      plot(DD_ENERGY2F_ENERGY1F, it - xTrace.begin(), traceNumST, *it);
	    for(vector<int>::iterator it = yTrace.begin(); it != yTrace.end(); it++) // 775
	      plot(DD_ENERGY2B_ENERGY1B, it - yTrace.begin(), traceNumST, *it);
	    traceNumST++;
	  } // endif(canFillST)
	// -----------------------------------------------------------------------------------------// END OF HANDLING SINGLE TRACES
