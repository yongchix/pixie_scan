...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(DD_CORRELATION_MATRIX_100F, 4028, 4028, "correlated decays in 30ms - F");// 707
  DeclareHistogram2D(DD_CORRELATION_MATRIX_100B, 4028, 4028, "correlated decays in 30ms - B");// 708
  //------------------------------------------------------------------------
...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  cout << "\n ===> Processing " << eventCnt <<  " event::DSSD " << endl; // by Yongchi Xiao; 06/15/2015 @
  ...
  for(vector<ChanEvent*>::const_iterator it = NaIEvents.begin(); it != NaIEvents.end(); it++)
    {
      ...
      // figure out order of processing
      /*
      if(eventCnt == 0)
	cout << number << ", "
	     << calEnergy << ", "
	     << setprecision(15) << naiTime << "\n";
      */
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
       ...
       it != xyEventsTMatch_.end(); ++it)
    {
      ...
      // test
      cout << " > DSSD hits: " << xEnergy << ", " << xTime << endl;
	...
	if( //xEnergy < 2730 && yEnergy < 2730
	    ...
	    && yEnergy > 0 && yEnergy < 3000 // high energy cut change to 3000 ch
	    ...
	    )          
	  {
		...
		chain[1][x][y].time = time;
	      ...
	      if( (time - chain[1][x][y].time) >= 0  
		  && (time - chain[1][x][y].time)*pow(10, -2) <= 600 // allow a time window = 200us
		  )
		{
		  ...
		  // qualified 2nd signal is plotted;

		  //--------------------------------- LOOK AT DIFFERENT ZONES IN CORR. MATRIC ---------------------------------
		  /*
		  if( (energy1F - 780)*(energy1F - 979) < 0 
		      && (xEnergy - 830)*(xEnergy - 1060) < 0
		      ) // zoneXe;
		    //   xc 780 979
		    //   yc 830 1060

		    {
		      plot(D_LOGDECAYTIME_0, 10*log((time - chain[1][x][y].time)*pow(10, -5))/log(10.) ); // 709
		      //outfile << setprecision(15) << time << "; " 
		      //      << setprecision(15) << xEnergy << "; "
		      //      << " <= DSSD SUMMED SIGNAL(107Te->103Sn)" 
		      //      << endl;//$

		      //outfile << "There are " << vecNaiEvents.size() << " NaI events in this raw events. " << endl;//$

		      for(int i = 0; i < vecNaiEvents.size(); i++)
			{
			  if(vecNaiEvents.at(i).time - time <=0 )
			    continue; 
			  else // if gammas come after a DSSD signal
			    {
			      NaiEvent nev = vecNaiEvents.at(i);
			      ...
			      //outfile << setprecision(15) << nev.time 
			      //      << "; ch-" << setprecision(2) << nev.channel 
			      //      << "; E-" << setprecision(8) << nev.energy 
			      //      << endl;//$
			    }
 			}
		  ...
		  */
		  ...
		  // remember to clear the elements after plotting 707/708
		  chain[1][x][y] = ini;
  ...
  cout << "DSSD RAW EVENT " << eventCnt++ << " IS PROCESSED!  <===" << endl << endl; // by Yongchi Xiao; 06/14/2015 @
