...
void Dssd4JAEAProcessor::DeclarePlots(void)
{
  ...
  DeclareHistogram2D(DD_NAI_VS_DSSD_IN, 4028, 1024, "NaI - E vs. DSSD - E, in"); // 713
  DeclareHistogram2D(DD_NAI_VS_DSSD_OUT, 4028, 1024, "NaI - E vs. DSSD - E, out"); // 714
  //-----------------------------------------------------------------------------------------
	...
	if( abs((xEnergy - yEnergy)/xEnergy) < 0.035
	    ...
	    )
	  {
	    ...
	    chain[0][x][y].time = time;
	      ...
	      {
		if(xEnergy > (50000/3.93)){ // if E > 70MeV, take it as an implant
		  chain[0][x][y].energyF = xEnergy;
		  chain[0][x][y].energyB = yEnergy;
		  chain[0][x][y].time = time;
		}
		...
		if(xEnergy < 3000 && yEnergy < 3000 // a possible decay event, not specially for 113Cs
		   ...
		   )
		  {
			...
			if((time - vecNaiEvents.at(i).time)*pow(10, -2) < 30
			   && (time - vecNaiEvents.at(i).time) > 0) // time window = 20us, half-life of 113Cs is 16.7us;
			  {
			    ...
			    if(naiOk_)
			      groupNaiEvents[channel_] = vecNaiEvents.at(i); // fill a possible 511keV photon
				...
				&& abs(groupNaiEvents[0].time - groupNaiEvents[1].time) <= 3 
			      ...
			      {
				double time_ = groupNaiEvents[0].time;
				double energy_ = (groupNaiEvents[0].energy + groupNaiEvents[1].energy + groupNaiEvents[2].energy + groupNaiEvents[3].energy)/4.;

				//---for test---
				/*
				cout << setprecision(6) << xEnergy << ", "
				     << setprecision(6) << groupNaiEvents[0].energy << ", "
				     << setprecision(6) << groupNaiEvents[1].energy << "- "
				     << setprecision(6) << groupNaiEvents[2].energy << "- "
				     << setprecision(6) << groupNaiEvents[3].energy << "- "
				...
				plot(DD_NAI_VS_DSSD_IN, xEnergy, energy_); // 713
				...
				&& abs( (groupNaiEvents[4].energy - groupNaiEvents[6].energy)/groupNaiEvents[4].energy*100. < 3.5 ) // energy almost the same
			      ...
			      {
				double time_ = groupNaiEvents[4].time;
				...
				plot(DD_NAI_VS_DSSD_OUT, xEnergy, energy_); // 714
				...
				&& abs( (groupNaiEvents[5].energy - groupNaiEvents[7].energy)/groupNaiEvents[5].energy*100. < 3.5 ) // energy almost the same
			      ...
			      {
				double time_ = groupNaiEvents[5].time;
