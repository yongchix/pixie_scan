...
bool Dssd4JAEAProcessor::Process(RawEvent &event)
{
  ...
  for (vector< pair<StripEvent, StripEvent> >::iterator it =
	...
	if(isDecay){	  
	  ...
	  PixelEvent pe;
	  pe.AssignValue(xEnergy, yEnergy, time);
	  SimpleEvent xe, ye, se;
	  ...
	  // make pair for PIN-f and DSSD-f
	  double time_ = 1000;
	  double energy_ = -1;
	  pair<PixelEvent, SimpleEvent> pDecay;
	  for(vector<SimpleEvent>::iterator ip = vecPinF.begin();
	      ip != vecPinF.end();
	      ip++){
	    if(ip->time - pe.time > 0 && ip->time - pe.time < time_)
	      se.AssignValue(ip->time, 
			     ip->energy,
			     ip->channel,
			     "pin-f");	    
	  }if(se.time > 0)
	     pDecay = make_pair(pe, se); 
	  ...
	  if(chain[1][x][y].time == -1){ // 1st component not found yet                                                                                          
            ...
            if((time - chain[1][x][y].time)*Globals::get()->clockInSeconds() < 1 // time window = 1 sec
	       && time - chain[1][x][y].time > 0 // in case the clock jump backwards;                                                                            
	       && pDecay.second.time > 0
	       && pDecay.second.time - pDecay.first.time > 3){
	      ...
	      // plot stuff
	      plot(11, chain[1][x][y].energyF, pDecay.first.energyF); // 711
	      plot(13, pDecay.first.energyF, pDecay.second.energy); // 713
	      plot(14, pDecay.first.energyF, pDecay.second.time - pDecay.first.time + 10); // 714
	      
	      // output
	      
	      ofstream outfile;
	      outfile.open("DSSD_CorrMatrix.txt", std::iostream::out | std::iostream::app); 
	      outfile << chain[1][x][y].energyF << "  "
		      << chain[2][x][y].energyF << "  " 
		      << std::setprecision(15) << chain[2][x][y].time - chain[1][x][y].time << "  " 
		      << pDecay.second.energy << "  " 
		      << std::setprecision(15) << pDecay.second.time - chain[2][x][y].time << "  ";
	      for(vector< pair<int, double> >::iterator itn = pDecay.first.vecNaiInfo.begin();
		  itn != pDecay.first.vecNaiInfo.end();
		  itn++){
		outfile << itn->first << "  " 
			<< itn->second << "  ";
	      }outfile << endl;
